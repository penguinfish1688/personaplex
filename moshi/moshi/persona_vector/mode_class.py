...
def plot_attention_heatmap(
    ...
    *,
    ...
    cbar.set_label("Attention logit", fontsize=20)
    cbar.ax.tick_params(labelsize=14)
    ax_top.set_ylabel("Query token position", fontsize=20)
    ...
    ax_bot.set_ylabel("Absolute amplitude", fontsize=20)
    ax_bot.set_xlabel("Time (seconds)  [aligned with key-axis above]", fontsize=20)
...
def plot_attention_heatmap_at_turn_taking(root_dir, span=20, layer=-1):
    ...
    for anchor in anchors:
        ...
        cax = ax_top.inset_axes([1.01, 0.0, 0.018, 1.0])
        cbar = fig.colorbar(img, cax=cax)
        cbar.set_label("Attention logit (P5->5, P95->95)", fontsize=20)
        cbar.ax.tick_params(labelsize=14)
        ax_top.set_ylabel("Query offset (s)", fontsize=20)
        ax_top.tick_params(axis="both", labelsize=14)
        ...
        ax_bot.set_ylabel("Absolute amplitude", fontsize=20)
        ax_bot.set_xlabel(f"Relative time to {anchor} (s)", fontsize=20)
        ax_bot.tick_params(axis="both", labelsize=14)
        ax_bot.grid(True, axis="x", linestyle=":", linewidth=0.7, alpha=0.65)
        ax_bot.legend(loc="upper right", fontsize=13)
...
def plot_attention_by_subseqent_token_heatmap(
    ...
    cbar.set_label("Avg future attention weight to token t (vmin=P5, vmax=P95)", fontsize=20)
    cbar.ax.tick_params(labelsize=14)

    ax.set_xlabel("Token offset from interrupt_start", fontsize=20)
...
def _parse_colorbar_range(value: Optional[str]) -> Optional[tuple[float, float]]:
    """Parse HIGH,LOW or LOW,HIGH into matplotlib vmin/vmax order."""
    if value is None:
        return None
    parts = [part.strip() for part in value.split(",")]
    if len(parts) != 2 or not all(parts):
        raise ValueError(
            f"--colorbar-range must be two comma-separated numbers, got {value!r}"
        )
    a, b = (float(parts[0]), float(parts[1]))
    if a == b:
        raise ValueError(f"--colorbar-range endpoints must differ, got {value!r}")
...
def logit_lens_heatmap(
    ...
    colorbar_range: Optional[str] = None,
) -> None:
    """
    ...
    if not root.is_dir():
        raise FileNotFoundError(f"Root directory not found: {root}")
    fixed_colorbar_bounds = _parse_colorbar_range(colorbar_range)
    ...
    for anchor in anchors:
        ...
        for which in ("line1", "line2"):
            ...
            if fixed_colorbar_bounds is not None:
                vmin, vmax = fixed_colorbar_bounds
                colorbar_ticks = None
                colorbar_ticklabels = None
            elif anchor == "interrupt_start":
                vmin, vmax = -16.5, -5.5
                colorbar_ticks = [-16, -14, -12, -10, -8, -6]
                colorbar_ticklabels = [f"{tick:.1f}" for tick in colorbar_ticks]
            else:
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Colorbar saturation endpoints for --plot-logit-lens-heatmap, "
            "formatted as HIGH,LOW or LOW,HIGH, e.g. -6,-15."
        ),
    )
    args = ap.parse_args()
